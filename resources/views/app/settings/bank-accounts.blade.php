        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <x-app.card class="relative overflow-hidden group">
                    ...
                    <div class="flex items-start justify-between mb-4">
                        ...
                        <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <x-app.button @click="openModal({{ $account->toJson() }})" variant="secondary" size="icon-xs">
                            ...
                            <form action="{{ route('app.settings.bank-accounts.destroy', [$company, $account]) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                                @csrf
                                @method('DELETE')
                                <x-app.button type="submit" variant="secondary" size="icon-xs" class="hover:text-rose-500">
            ...
            @empty
                <div class="lg:col-span-3 text-center py-12 bg-slate-50 dark:bg-slate-900/50 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800">
                    <svg class="w-12 h-12 mx-auto mb-4 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 14v20m4-20v20m4-20v20m6-21a2 2 0 11-4 0 2 2 0 014 0zM4 10h16a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2v-8a2 2 0 012-2z"/></svg>
                    <p class="text-slate-500 font-medium">No bank accounts linked yet</p>
                    <x-app.button @click="openModal()" variant="primary" size="sm" class="mt-4">Connect Bank Account</x-app.button>
                </div>
            @endforelse

            @if($bankAccounts->count() > 0)
            <x-app.card @click="openModal()" class="flex items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-800 hover:border-violet-300 dark:hover:border-violet-800 transition-colors cursor-pointer group">
                <div class="flex flex-col items-center">
                    <div class="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 group-hover:bg-violet-100 group-hover:text-violet-600 transition-colors mb-2">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
                    </div>
                    <span class="text-xs font-bold text-slate-400 group-hover:text-violet-600 transition-colors uppercase tracking-wider">Add Account</span>
                </div>
            </x-app.card>
            @endif
        </div>
