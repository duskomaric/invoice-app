...
@section('title', 'Edit Quote - App')
@section('page-title', 'Edit Quote')
@section('page-subtitle', 'Update quote details and estimates')
@section('page-badge', 'Edit')

@section('header-actions')
<div class="flex items-center gap-2">
    <x-app.button href="{{ route('app.quotes.show', [$company, $quote]) }}" variant="secondary" size="sm" class="hidden sm:flex">
        Cancel
    </x-app.button>
...
@section('content')
<form id="quote-form" action="{{ route('app.quotes.update', [$company, $quote]) }}" method="POST"
    x-data="{
        items: @js($quote->items->map(fn($item) => [
            'id' => $item->id,
            'name' => $item->name,
        ...
        selectedClient: @json($quote->client),
        search: '',
        open: false,
    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <x-app.card class="lg:col-span-2 relative z-20">
                ...
                <div class="mt-4 relative" x-data="{ open: false, search: '{{ $quote->client->name }}', clients: @js($clients) }" @click.away="open = false">
            ...
            <x-app.card>
                ...
                <div class="space-y-3 mt-4">
                    <x-app.input label="Date" type="date" name="date" value="{{ $quote->date->format('Y-m-d') }}" />
                    <x-app.input label="Valid Until" type="date" name="valid_until" value="{{ $quote->valid_until->format('Y-m-d') }}" />
                    <div>
                        ...
                        <select name="currency" class="w-full h-10 px-3 rounded-xl bg-white dark:bg-slate-800 border-none ring-1 ring-slate-200 dark:ring-slate-700 focus:ring-2 focus:ring-violet-500 transition-all text-sm">
                            @foreach($currencies as $currency)
                                <option value="{{ $currency->code }}" {{ $quote->currency === $currency->code ? 'selected' : '' }}>{{ $currency->code }} - {{ $currency->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="language" class="w-full h-10 px-3 rounded-xl bg-white dark:bg-slate-800 border-none ring-1 ring-slate-200 dark:ring-slate-700 focus:ring-2 focus:ring-violet-500 transition-all text-sm">
                            <option value="en" {{ $quote->language === 'en' ? 'selected' : '' }}>English</option>
                            <option value="hr" {{ $quote->language === 'hr' ? 'selected' : '' }}>Croatian</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="status" class="w-full h-10 px-3 rounded-xl bg-white dark:bg-slate-800 border-none ring-1 ring-slate-200 dark:ring-slate-700 focus:ring-2 focus:ring-violet-500 transition-all text-sm">
                            @foreach(['draft', 'sent', 'accepted', 'declined', 'expired'] as $status)
                                <option value="{{ $status }}" {{ $quote->status === $status ? 'selected' : '' }}>{{ ucfirst($status) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
        ...
        <x-app.card class="relative z-10">
            <div class="flex items-center justify-between mb-4">
                <x-app.section-header title="Quote Items" subtitle="Add products or services to this document" icon="list-bullet" variant="primary" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
                <div>
                    ...
                    <textarea name="notes" rows="4" class="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700 focus:ring-2 focus:ring-violet-500 transition-all text-xs text-slate-600 placeholder-slate-400" placeholder="Add payment instructions, terms, or other details...">{{ $quote->notes }}</textarea>
                </div>
                <div class="space-y-2">
