<x-app-layout>
    <x-slot name="title">Create Proforma - {{ $company->name }}</x-slot>

    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-gray-900">Create Proforma</h1>
        <p class="text-sm text-gray-500">Next number: {{ $previewNumber }}</p>
    </div>

    <form action="{{ route('app.proformas.store', $company) }}" method="POST" x-data="documentForm()">
        @csrf
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white shadow rounded-lg p-6">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="date" name="due_date" value="{{ now()->addDays(14)->format('Y-m-d') }}" class="w-full h-10 px-3 text-sm border rounded-md">
                        </div>
                        <div>
            ...
            <div class="bg-white shadow rounded-lg p-6 h-fit sticky top-6">
                <button type="submit" class="w-full mb-3 px-4 py-2 text-sm font-medium text-white bg-neutral-950 rounded-md">Create Proforma</button>
                <a href="{{ route('app.proformas.index', $company) }}" class="w-full block text-center px-4 py-2 text-sm text-gray-700 border rounded-md">Cancel</a>
            </div>
        </div>
    </form>
