<x-app-layout>
    <x-slot name="title">Edit Contract {{ $contract->formatted_number }}</x-slot>

    <div class="mb-6"><h1 class="text-2xl font-semibold text-gray-900">Edit Contract {{ $contract->formatted_number }}</h1></div>

    <form action="{{ route('app.contracts.update', [$company, $contract]) }}" method="POST" 
          x-data="documentForm({{ json_encode($contract->items->map(fn($i) => ['name' => $i->name, 'quantity' => $i->quantity, 'unit_price' => $i->unit_price / 100, 'total' => $i->total / 100])) }})">
        @csrf @method('PUT')
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="client_id" required class="w-full h-10 px-3 text-sm border rounded-md">
                                @foreach($clients as $c)<option value="{{ $c->id }}" {{ $contract->client_id == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>@endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Status</label>
                            <select name="status" required class="w-full h-10 px-3 text-sm border rounded-md">
                                <option value="draft" {{ $contract->status->value === 'draft' ? 'selected' : '' }}>Draft</option>
                                <option value="active" {{ $contract->status->value === 'active' ? 'selected' : '' }}>Active</option>
                            </select>
                        </div>
                        <div><label class="block text-sm font-medium mb-1">Date</label><input type="date" name="date" value="{{ $contract->date?->format('Y-m-d') }}" required class="w-full h-10 px-3 text-sm border rounded-md"></div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Currency</label>
                            <select name="currency" required class="w-full h-10 px-3 text-sm border rounded-md">
                                @foreach($currencies as $c)<option value="{{ $c->code }}" {{ $contract->currency == $c->code ? 'selected' : '' }}>{{ $c->code }}</option>@endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Language</label>
                            <select name="language" required class="w-full h-10 px-3 text-sm border rounded-md">
                                <option value="en" {{ $contract->language?->value === 'en' ? 'selected' : '' }}>English</option>
                                <option value="bs" {{ $contract->language?->value === 'bs' ? 'selected' : '' }}>Bosnian</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white shadow rounded-lg p-6"><textarea name="notes" rows="3" class="w-full px-3 py-2 text-sm border rounded-md">{{ $contract->notes }}</textarea></div>
            </div>
            ...
            <div class="bg-white shadow rounded-lg p-6 h-fit sticky top-6">
                ...
                <a href="{{ route('app.contracts.show', [$company, $contract]) }}" class="w-full block text-center px-4 py-2 text-sm text-gray-700 border rounded-md">Cancel</a>
            </div>
        </div>
    </form>
